...
class WebSocketService {
  ...
  Future<void> connect(String token) async {
    ...
    try {
      ...
      debugPrint('[WEBSOCKET] Using token: ${token.substring(0, 20)}...');

      _socket = io.io(
        backendUrl,
        io.OptionBuilder()
            .setTransports(
                ['polling', 'websocket']) // Polling first for web compatibility
            .enableAutoConnect()
            .enableReconnection()
  ...
  void _processOfflineQueue() {
    ...
    debugPrint(
        '[WEBSOCKET] Processing ${_offlineQueue.length} queued messages');
